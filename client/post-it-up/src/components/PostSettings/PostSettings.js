...
export default function PostSettings({
  ...
  const changePostPrivacy = (isPublic) => {
    dispatch(updatePostPrivacy({ postId: post._id, public: !isPublic })).then(
      (resp) => {
        if (resp.payload && resp.payload.message) {
          dispatch(fetchPostsByUser(userId));
          alert(
            `Post successfully ${isPublic ? "unpublished!" : "published!"}`
          );
        }
        onClose();
      }
  ...
  return (
    ...
    >
      <SpeedDial
        ...
        sx={{ position: "absolute", bottom: 21, right: -20 }}
